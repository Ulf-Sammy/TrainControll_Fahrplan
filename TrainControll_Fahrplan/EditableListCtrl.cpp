...
void CEditableListCtrl::OnLButtonDown(UINT nFlags, CPoint point, int* Index)
{	
	int index;	
	...
	int colnum;	
	index = GetRowFromPoint(point, &colnum);
	*Index = index;
	// index = 1;
	...
	{		
		...
		if( (GetItemState( index, flag ) & flag) == flag && (colnum == 2 || colnum == 3))		
		{			
...
void CEditableListCtrl::OnKeydown(int nZeile, int Spalte)
{
	EditSubLabel(nZeile, Spalte);
}
