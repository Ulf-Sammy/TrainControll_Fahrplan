...
BEGIN_MESSAGE_MAP(CDlg_CV_Action, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_WRITE, &CDlg_CV_Action::OnBnClickedButtonWrite)
...
{ //Read Data
		UpdateData(true);
		byte Info = Zug_Data->PRG_Lok_CV(false, CV_Nr, &newCV_Wert);
		if (0 == Info )
		{
			if (newCV_Wert != altCV_Wert)
			{
				m_NewData = true;
				altCV_Wert = newCV_Wert;
			}
			m_CV_Info.Format(_T("gelesen CV Nr.: %3d   =  %3d"), CV_Nr, newCV_Wert);
		}
		else
		{
			if (Info == 1) { m_CV_Info.Format(_T("Error ! keinen Dekoder gefunden"));		}
			if (Info == 2) { m_CV_Info.Format(_T("Error ! Kurzschluß erkannt     ")); }
		}
		UpdateData(false);
}

void CDlg_CV_Action::OnBnClickedButtonWrite()
{ // Write Data
		UpdateData(true);
		if (0 == Zug_Data->PRG_Lok_CV(true, CV_Nr, &newCV_Wert))
		{
			if (newCV_Wert != altCV_Wert)
			{
				m_NewData = true;
...
BOOL CDlg_CV_Action::OnInitDialog()
{
	...
	m_Lok_Info.Format(_T(" Werte der Lok : %s "), (LPCTSTR)Zug_Data->Get_Lok_Name(Zug_Data->Get_Prog_Zug())); //MeineZüge[].Name);
