...
//	BlockData = Gleise->Get_DebugData(Block_Nr);
	...
	/*BlockNr.Format(_T("Block : %d"), BlockData.Block_Nr);
	...
	for (int i = 0; i < 8 ; i++)
	{
		if (i < BlockData.EinBlock.size())
			Ein_BlockNr[i].Format(_T("<=: %2d "), BlockData.EinBlock[i]);
		else
			Ein_BlockNr[i].Empty();
		if (i < BlockData.AusBlock.size())
			Aus_BlockNr[i].Format(_T(" %2d :=>"), BlockData.AusBlock[i]);
		else
			Aus_BlockNr[i].Empty();
	}
...
BEGIN_MESSAGE_MAP(CDlg_Block_Info, CDialogEx)
	...
	ON_NOTIFY(UDN_DELTAPOS, IDC_SPIN1, &CDlg_Block_Info::OnDeltaposSpin1)
END_MESSAGE_MAP()
...
void CDlg_Block_Info::OnPaint()
{
	...
	pDC = &memDC;
	Zeichen_BlockInfo(CRect(2,20,282,300));
}
...
void CDlg_Block_Info::Zeichen_BlockInfo(CRect R)
{
	...
	pDC->Rectangle(R);
	pDC->TextOutW(120, 24, BlockNr);

	for (int i = 0; i < 8; i++)
	{
		pDC->TextOutW(5, 43 + (14 * i), Ein_BlockNr[i]);
		pDC->TextOutW(250, 43 + (14 * i), Aus_BlockNr[i]);
		if (!Ein_BlockNr[i].IsEmpty())
		{
			pDC->MoveTo(38, 50 + (14 * i));
			pDC->LineTo(100, 50);
		}
		if (!Aus_BlockNr[i].IsEmpty())
		{
			pDC->MoveTo(180, 50);
			pDC->LineTo(250, 50 + (14 * i));
		}

	}

	pDC->Rectangle(100, 40, 180, 60);
	pDC->MoveTo(   2, 160);
	pDC->LineTo( 282, 160);
...
void CDlg_Block_Info::OnDeltaposSpin1(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMUPDOWN pNMUpDown = reinterpret_cast<LPNMUPDOWN>(pNMHDR);
	Block_Nr = pNMUpDown->iPos;
	NeueDaten();
