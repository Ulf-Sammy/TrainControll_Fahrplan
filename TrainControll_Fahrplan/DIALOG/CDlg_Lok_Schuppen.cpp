...
BOOL CDlg_Lok_Schuppen::Init()
{
	...
	//for (auto& Lok : APP->meineLoks.ZugHomeDaten())
	//{
...
void CDlg_Lok_Schuppen::OnPaint()
{
	...
	for (int i = 0; i < 2; i++)
	{
		switch (Status) //Gleise->GetStatus_Block(Strecke_Block[i], &LocName))
		{
	...
	for (int i = 0; i < MaxAbstellGleise; i++)
	{
		switch (Status) //Gleise->GetStatus_Block(Gleis_Block[i], &LocName, &Lok_Richtung))
		{
		case BlockStatus::Frei:
			pDC->SelectObject(theApp.Gleis_Frei);
			break;
		...
		if (true) // Gleise->isPower_onBlock(Gleis_Block[i]))
			pDC->SelectObject(theApp.Gleis_Frei);
		else
		...
		if (true) //!Gleise->Get_LokName_in_Block(Gleis_Block[i]).IsEmpty())
		{
			if (Lok_Richtung)
...
void CDlg_Lok_Schuppen::WeichenWeg_schalten(byte I)
{
	...
	{
		//BlockNet->Send_WeichenData(Weiche_Nummer[i]);
	}
}
...
BEGIN_MESSAGE_MAP(CDlg_Lok_Schuppen, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON2, &CDlg_Lok_Schuppen::OnBnClickedButton2)
END_MESSAGE_MAP()
...
void CDlg_Lok_Schuppen::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	{
		bool SchuppenTor_auf = true; //!Gleise->Get_Door_open();
	...
	if (true) //Gleise->Get_Door_open())
	{
		for (int i = 0; i < MaxAbstellGleise; i++)
		{
			if (Gleis_Platz[i].PtInRect(point))
			{
				...
				{ 
					//Gleise->Schalte_GleisPower(Gleis_Block[i], false);
					last_On_Block = 0;
				}
				else
				{
					WeichenWeg_schalten(i);
					//Gleise->Schalte_GleisPower(last_On_Block, false);
					//Gleise->Schalte_GleisPower(Gleis_Block[i], true);
	...
	if (Zeige_Comobox)
	{
		for (int i = 0; i < MaxAbstellGleise; i++)
		{
			...
			{
				if (true) //Gleise->Get_Zug_Point(i + 33) != nullptr)
				{
					//Gleise->Get_Zug_Point(i + 33)->Blick = !Gleise->Get_Zug_Point(i + 33)->Blick;
					if (true) //Gleise->Get_Zug_Point(i + 33)->Blick)
					{
...
void CDlg_Lok_Schuppen::OnBnClickedButton2()
{
	if (Power_ProgGleis)
	{
		SetDlgItemTextW(IDC_BUTTON2, L"Power on");
		Power_ProgGleis = false;
	}
	else 
	{
		SetDlgItemTextW(IDC_BUTTON2, L"Power off");
		Power_ProgGleis = true;
	}
	//Gleise->Schalte_Relais(0,Power_ProgGleis);
}
