...
void CTrainControll_FahrplanDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATIC_MEGA, InfoGiga);
	DDX_Control(pDX, IDC_STATIC_LVZ2, InfoLVZ200);
	DDX_Control(pDX, IDC_STATIC_WIFI, InfoWifi); 
...
//##########################################################################
	...
	StromKurve.Init();
	InfoGiga.Init(sizeof(LED_Images), LED_Images);
	InfoGiga.Set_Status(0, _T("no conect to Server !"));
	InfoLVZ200.Init(sizeof(LED_Images), LED_Images);
	InfoWifi.Init(sizeof(LED_Images), LED_Images);
	InfoWifi.Set_Status(0, _T("no conect to Wifi !"));
...
void CTrainControll_FahrplanDlg::Set_Train_Run_DLG()
{
	...
	for (int index = 0; index < 8; index++)
	{
		...
		Anlagen_Daten.Gleis_Data.GetStatus_Block(Lok_Block, &LokName);
		if (LokName.IsEmpty())
		{
...
void CTrainControll_FahrplanDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	for (size_t i = 0; i < 4; i++)
	{
		...
		{
		case 0:
			pWnd = GetDlgItem(IDC_STATIC_POWER);
			break;
		case 1:
			pWnd = GetDlgItem(IDC_STATIC_MEGA);
			break;
		case 2:
			pWnd = GetDlgItem(IDC_STATIC_WIFI);
			break;
		case 3:
			pWnd = GetDlgItem(IDC_STATIC_LVZ2);
			break;
		default:
			break;
		}
		...
		if (pWnd)
		{
			pWnd->GetWindowRect(&rect); // Bildschirmkoordinaten
			ScreenToClient(&rect);   
			if ((rect.left < point.x) && (rect.right > point.x))
			{
				if ((rect.bottom > point.y) && (rect.top < point.y))
				{
					...
					case 0:
						...
						break;
					case 1:
						if (Anlagen_Daten.Server_da)
						{
							Anlagen_Daten.Wifi_Client.disconnect_Server();
							Anlagen_Daten.Server_da = false;
						}
						else
						{
							Anlagen_Daten.Wifi_Client.connect_Server();
							Anlagen_Daten.Server_da = true;
						}
						Anlagen_Daten.DoUpdateWifi = true;
						Anlagen_Daten.DoUpdateGiga = true;
					case 2:
						break;
					case 3:
						Anlagen_Daten.Connect_LZV200();
						Anlagen_Daten.DoUpdateLZV = true;
						break;
					}
				}
			}
...
{ // wie schalte ich zu progrmmodus um 
	...
	Invalidate();
	Dlg.SetPointer(&Anlagen_Daten.meineLoks);
	Dlg.DoModal();
	//XpressNet.Sende_Setto_Prog(false); //ChangeMode(Fahren);
	Invalidate();
	Anlagen_Daten.meineLoks.Set_aktiveLok_FuntiontoRun();
}
...
void CTrainControll_FahrplanDlg::OnSetupMeinezugliste()
{
	...
	Dlg.Set_TrainData(&Anlagen_Daten.meineLoks);
	Dlg.DoModal();
}
...
void CTrainControll_FahrplanDlg::DoUpdate()
{
	// Update the dialog with the latest data
	//InfoGleisBild.Invalidate();
	// StromKurve.Invalidate();
	if (Anlagen_Daten.DoUpdateWifi)
	{ 
		InfoWifi.Set_Status(Anlagen_Daten.LisentoServer, Anlagen_Daten.Signalstaerke_Wifi);
		InfoGiga.Set_Status(Anlagen_Daten.Server_da, Anlagen_Daten.NR_IP_Client);
		Anlagen_Daten.DoUpdateWifi = false;
	}
	if (Anlagen_Daten.DoUpdateGiga)
	{
		if (Anlagen_Daten.Server_da)
		{
			InfoGiga.Set_Status(Anlagen_Daten.Server_da, Anlagen_Daten.NR_IP_Client);
		...
		{
			InfoGiga.Set_Status(Anlagen_Daten.Server_da, _T("ab gemeldet"));
		}
		Anlagen_Daten.DoUpdateGiga = false;
	}
	if( Anlagen_Daten.DoUpdateLZV)
	{
		if(Anlagen_Daten.LZV_da)
		  InfoLVZ200.Set_Status(1, _T(" LZV200 verbunden"));
		else
			InfoLVZ200.Set_Status(0, _T("no LZV200 conected !"));
		Anlagen_Daten.DoUpdateLZV = false;
	}
	if (Anlagen_Daten.DoUpdatePower)
	{
		InfoPower.Set_Status(Anlagen_Daten.Gleispower);
	}
	//InfoModus.Invalidate();
	//InfoMelder.Invalidate();
	//InfoWeiche.Invalidate();
	/*
	for (byte i = 0; i < 8; i++)
	{
		if (pDlgTrainRun[i] != NULL)
			pDlgTrainRun[i]->Invalidate();
	}
	if (pDlgTrainRunCam != NULL)
		pDlgTrainRunCam->Invalidate();
	if (pDlgSchuppen != NULL)
		pDlgSchuppen->Invalidate();
	if (pDlgBlockInfo != NULL)
		pDlgBlockInfo->Invalidate();
	if (pDlgComListe != NULL)
		pDlgComListe->Invalidate();
	*/
}
...
void CTrainControll_FahrplanDlg::OnClose()
{
	...
	Sleep(500);
	Anlagen_Daten.close_Comunikation();
	Sleep(500);
	for (int i = 0; i < 8; i++)
	{
		if (pDlgTrainRun[i] != NULL)
